...
import {
  getForgeProfile, saveForgeProfile, getForgeTasks, saveForgeTask,
  updateForgeTask, getNewsCache, saveNewsCache, getPendingTask,
...
function TaskCard({ task, groqKey, onDone, onSkip }: {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setResult({ score: 0, feedback: "Error: " + e.message, correct: false });
    } finally { setChecking(false); }
  }

  return (
    <div style={{ background:"#f7f7f8", border:"1px solid #e5e5ea", borderRadius:"12px", padding:"15px" }}>
      ...
      {task.type === "link" ? (
        ...
        <div style={{ background: result.correct ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.08)", border:`1px solid ${result.correct ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.3)"}`, borderRadius:"10px", padding:"12px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
            <div style={{ width:44, height:44, borderRadius:"50%", border:`2px solid ${result.correct ? "#22c55e" : "#ef4444"}`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Mono',monospace", fontWeight:700, color: result.correct ? "#22c55e" : "#ef4444" }}>
              {result.score}
            </div>
...
function KeysSetup({ onSave }: { onSave: () => void }) {
  ...
  function handle() {
    saveForgeKeys({ tavily: tavily.trim(), groq: groq.trim() });
    onSave();
  }

  const inputStyle = { width:"100%", padding:"10px 12px", background:"#f7f7f8", border:"1px solid #e5e5ea", borderRadius:"8px", fontFamily:"'DM Mono',monospace", fontSize:"0.8rem", color:"#111", outline:"none", boxSizing:"border-box" as const, marginBottom:12 };
  ...
  return (
    <div style={{ padding:"20px" }}>
      ...
      <input type="password" value={tavily} onChange={e => setTavily(e.target.value)} placeholder="tvly-..." style={inputStyle} />
      <label style={{ fontSize:"0.75rem", color:"#666", fontFamily:"'DM Mono',monospace", display:"block", marginBottom:6 }}>GROQ API KEY</label>
      <input type="password" value={groq} onChange={e => setGroq(e.target.value)} placeholder="gsk_..." style={inputStyle} />
...
export function ForgeAvatar({ onClick, hasPending }: { onClick: () => void; hasPending: boolean }) {
  return (
    <button onClick={onClick}
      style={{ position:"fixed", bottom:"80px", left:"20px", zIndex:200, width:"52px", height:"52px", borderRadius:"50%", background:"#111", border:"2px solid #6366f1", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 20px rgba(99,102,241,0.3)", transition:"transform 0.2s" }}
      onMouseEnter={e => (e.currentTarget as HTMLElement).style.transform="scale(1.1)"}
      onMouseLeave={e => (e.currentTarget as HTMLElement).style.transform="scale(1)"}>
      <span style={{ fontSize:"22px" }}>⚒️</span>
      {hasPending && (
        <div style={{ position:"absolute", top:2, right:2, width:10, height:10, background:"#f59e0b", borderRadius:"50%", border:"2px solid #111" }} />
      )}
    </button>
  );
}

export default function ForgePanel({ onClose, articleContext }: {
  onClose: () => void;
  articleContext?: { title: string; tag: string; text: string; url: string } | null;
}) {
  ...
  const [setupDone, setSetupDone] = useState(mkdir -p src/components/forge && cat > src/components/forge/index.tsx << 'EOF'
"use client";
import { useState, useEffect, useCallback } from "react";
import {
  getForgeProfile, saveForgeProfile, getForgeTasks, saveForgeTask,
...
EOF(getForgeKeys().tavily && getForgeKeys().groq));
  ...
  async function generateTask() {
    ...
    } catch (e: any) { console.error("Forge generate:", e.message); }
  ...
  const tabStyle = (t: string) => ({
    flex: 1, padding:"8px", background: tab === t ? "#6366f1" : "none",
    color: tab === t ? "#fff" : "#999", border:"none", borderRadius:"8px",
    cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:600, fontSize:"0.78rem",
  });
    ...
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(10,10,11,0.7)", backdropFilter:"blur(8px)", display:"flex", alignItems:"flex-end" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:"480px", margin:"0 auto", background:"#fff", borderRadius:"20px 20px 0 0", maxHeight:"82vh", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ display:"flex", gap:4, padding:"8px 16px", background:"#f7f7f8", flexShrink:0 }}>
          <button style={tabStyle("intel")} onClick={() => setTab("intel")}>📡 Intel</button>
          <button style={tabStyle("tasks")} onClick={() => setTab("tasks")}>⚒️ Tasks {pending.length > 0 && `(${pending.length})`}</button>
        ...
        {tab === "intel" && (
          <div style={{ flex:1, overflowY:"auto", padding:"16px 20px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
              <span style={{ fontSize:"11px", fontFamily:"'DM Mono',monospace", color:"#999" }}>
                {getNewsCache() ? "cached · " + new Date(JSON.parse(localStorage.getItem("forge_news_cache")||"{}").fetchedAt||0).toLocaleTimeString() : "live fetch"}
              </span>
            ...
            {CAT_ORDER.map(cat => {
              ...
              return (
                <div key={cat} style={{ marginBottom:20 }}>
                  <p style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.82rem", marginBottom:10, color:"#111" }}>{CAT_LABEL[cat]}</p>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"16px 20px" }}>
            {generating && (
              <div style={{ textAlign:"center", padding:"20px", background:"rgba(99,102,241,0.06)", border:"1px solid rgba(99,102,241,0.2)", borderRadius:"10px", marginBottom:14 }}>
                <p style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.8rem", color:"#6366f1" }}>⚒️ Generating task from article...</p>
              </div>
            )}
            {articleContext && !generating && (
              <button onClick={generateTask}
                ...
                ↺ Regenerate Task for Current Article
              </button>
            )}
            {pending.length === 0 && done.length === 0 && !generating && (
              <div style={{ textAlign:"center", padding:"40px 20px", color:"#999" }}>
                ...
                <p style={{ fontFamily:"'DM Mono',monospace", fontSize:"0.75rem" }}>Read an article and FORGE will generate a task</p>
              </div>
            )}
            {pending.map(task => (
        ...
        {tab === "setup" && (
          <div style={{ flex:1, overflowY:"auto" }}>
            ...
            <div style={{ padding:"0 20px 20px" }}>
              <p style={{ fontFamily:"'DM Mono',monospace", fontSize:"11px", color:"#999", marginBottom:10 }}>WEAK AREAS (skipped most)</p>
